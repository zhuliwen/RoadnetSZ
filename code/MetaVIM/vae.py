...
from models.decoder import StateTransitionDecoder, RewardDecoder, NeighborStateTransitionDecoder, NeighborRewardDecoder, TaskDecoder
...
class VaribadVAE:
    ...
    def __init__(self, args, logger, get_iter_idx):
        ...
        # initialise the decoders (returns None for unused decoders with neighbor's action
        if args.use_neighbor:
            self.state_decoder_neighbor, self.reward_decoder_neighbor = self.initialise_decoder_neighbor()
        ...
        if not self.args.disable_decoder:
            ...
            if self.args.use_neighbor:
                if self.args.decode_state_neighbor:
                    decoder_params.extend(self.state_decoder_neighbor.parameters())
                if self.args.decode_reward_neighbor:
                    decoder_params.extend(self.reward_decoder_neighbor.parameters())
    ...
    def compute_intrinsic_reward(self,
                                 ...
                                 dec_next_obs,
                                 dec_actions,
                                 dec_actions_neighbor
                                 ):
        pass
        '''
        1. error(state_reconstruction vs state_reconstruction_neighbor)
        2. error(reward_reconstruction vs reward_reconstruction_neighbor)
        '''
        # error 1
        if self.args.state_pred_type == 'deterministic':
            obs_reconstruction = self.state_decoder(dec_embedding, dec_prev_obs, dec_actions)
            obs_reconstruction_neighbor = self.state_decoder_neighbor(dec_embedding, dec_prev_obs, dec_actions, dec_actions_neighbor)

            error1 = (obs_reconstruction - obs_reconstruction_neighbor).pow(2).mean(dim=1)
            ...
            state_pred_neighbor = self.state_decoder_neighbor(dec_embedding, dec_prev_obs, dec_actions)
            state_pred_mean_neighbor = state_pred_neighbor[:, :state_pred_neighbor.shape[1] // 2]
            state_pred_std_neighbor = torch.exp(0.5 * state_pred_neighbor[:, state_pred_neighbor.shape[1] // 2:])
            n = torch.distributions.normal.Normal(state_pred_mean_neighbor, state_pred_std_neighbor)

            error1 = -m.log_prob(n).mean(dim=1)
        ...
        if self.args.multihead_for_reward:
            if self.args.rew_pred_type == 'bernoulli' or self.args.rew_pred_type == 'categorical':
                ...
                p_rew_neighbor = self.reward_decoder_neighbor(dec_embedding, None)
                ...
                rew_pred_neighbor = p_rew_neighbor.gather(dim=-1, index=indices)
                error2 = F.binary_cross_entropy(rew_pred, rew_pred_neighbor, reduction='none').mean(dim=-1)
                ...
                error2 = F.mse_loss(p_rew.gather(1, indices.reshape(-1, 1)), p_rew_neighbor.gather(1, indices.reshape(-1, 1)), reduction='none').mean(
                    dim=1)
            else:
                raise NotImplementedError
        else:
            if self.args.rew_pred_type == 'bernoulli':
                ...
                rew_pred_neighbor = self.reward_decoder_neighbor(dec_embedding, dec_next_obs)
                error2 = F.binary_cross_entropy(rew_pred, rew_pred_neighbor, reduction='none').mean(dim=1)
            elif self.args.rew_pred_type == 'deterministic':
                rew_pred = self.reward_decoder(dec_embedding, dec_next_obs, dec_prev_obs, dec_actions)
                rew_pred_neighbor = self.reward_decoder_neighbor(dec_embedding, dec_next_obs, dec_prev_obs, dec_actions, dec_actions_neighbor)
                error2 = (rew_pred - rew_pred_neighbor).pow(2).mean(dim=1)
                ...
                rew_pred_neighbor = self.reward_decoder_neighbor(dec_embedding, dec_next_obs, dec_prev_obs, dec_actions, dec_actions_neighbor).mean(dim=1)
                rew_pred_mean_neighbor = rew_pred_neighbor[:, :rew_pred_neighbor.shape[1] // 2]
                rew_pred_std_neighbor = torch.exp(0.5 * rew_pred_neighbor[:, rew_pred_neighbor.shape[1] // 2:])
                n = torch.distributions.normal.Normal(rew_pred_mean_neighbor, rew_pred_std_neighbor)

                error2 = -m.log_prob(n)
            else:
                raise NotImplementedError

        intrinsic_reward = (error1 + error2) * self.args.intrinsic_reward_weight
    ...
    def initialise_decoder_neighbor(self):
        ...
        if self.args.decode_reward_neighbor:
            # initialise reward decoder for VAE with action of neighbor
            reward_decoder_neighbor = NeighborRewardDecoder(
                layers=self.args.reward_decoder_layers,
                latent_dim=latent_dim,
                #
                state_dim=self.args.obs_dim,
                ...
                input_prev_state=self.args.input_prev_state,
                input_action=self.args.input_action,
                input_action_neighbor=self.args.input_action_neighbor,
            ).to(device)
        else:
            reward_decoder_neighbor = None

        if self.args.decode_state_neighbor:
            # initialise state decoder for VAE
            state_decoder_neighbor = NeighborStateTransitionDecoder(
                latent_dim=latent_dim,
                layers=self.args.state_decoder_layers,
                action_dim=self.args.action_dim,
                action_embed_dim=self.args.action_embedding_size,
            ...
            state_decoder_neighbor = None

        return state_decoder_neighbor, reward_decoder_neighbor
    ...
    def compute_state_reconstruction_loss_neighbor(self, dec_embedding, dec_prev_obs, dec_next_obs, dec_actions, dec_actions_neighbor,
                                                   return_predictions=False):
        # make some predictions and compute individual losses
        if self.args.state_pred_type == 'deterministic':
            obs_reconstruction = self.state_decoder_neighbor(dec_embedding, dec_prev_obs, dec_actions, dec_actions_neighbor)
            loss_state = (obs_reconstruction - dec_next_obs).pow(2).mean(dim=1)
        elif self.args.state_pred_type == 'gaussian':
            state_pred = self.state_decoder_neighbor(dec_embedding, dec_prev_obs, dec_actions, dec_actions_neighbor)
            state_pred_mean = state_pred[:, :state_pred.shape[1] // 2]
            state_pred_std = torch.exp(0.5 * state_pred[:, state_pred.shape[1] // 2:])
            m = torch.distributions.normal.Normal(state_pred_mean, state_pred_std)
            loss_state = -m.log_prob(dec_next_obs).mean(dim=1)
    ...
    def compute_rew_reconstruction_loss_neighbor(self, dec_embedding,
                                        dec_prev_obs, dec_next_obs,
                                        dec_actions, dec_rewards,
                                        dec_actions_neighbor,
                                        return_predictions=False):
        ...
        if self.args.multihead_for_reward:
            if self.args.rew_pred_type == 'bernoulli' or self.args.rew_pred_type == 'categorical':
                # loss for the data we fed into encoder
                p_rew = self.reward_decoder_neighbor(dec_embedding, None)
                env = gym.make(self.args.env_name)
                indices = env.task_to_id(dec_next_obs).to(device)
                if indices.dim() < p_rew.dim():
                    indices = indices.unsqueeze(-1)
            ...
            if self.args.rew_pred_type == 'bernoulli':
                rew_pred = self.reward_decoder_neighbor(dec_embedding, dec_next_obs)
                loss_rew = F.binary_cross_entropy(rew_pred, (dec_rewards == 1).float(), reduction='none').mean(dim=1)
            elif self.args.rew_pred_type == 'deterministic':
                rew_pred = self.reward_decoder_neighbor(dec_embedding, dec_next_obs, dec_prev_obs, dec_actions, dec_actions_neighbor)
                loss_rew = (rew_pred - dec_rewards).pow(2).mean(dim=1)
            elif self.args.rew_pred_type == 'gaussian':
                rew_pred = self.reward_decoder_neighbor(dec_embedding, dec_next_obs, dec_prev_obs, dec_actions, dec_actions_neighbor).mean(dim=1)
                rew_pred_mean = rew_pred[:, :rew_pred.shape[1] // 2]
                rew_pred_std = torch.exp(0.5 * rew_pred[:, rew_pred.shape[1] // 2:])
                m = torch.distributions.normal.Normal(rew_pred_mean, rew_pred_std)
                loss_rew = -m.log_prob(dec_rewards)
    ...
    def compute_vae_loss(self, update=False):
        ...
        len_encoder, trajectory_lens, vae_actions_neighbor = self.rollout_storage.get_batch(num_rollouts=self.args.vae_batch_num_trajs,
                                                                      num_enc_len=self.args.vae_batch_num_enc_lens)
        vae_actions_neighbor = vae_actions_neighbor.detach()
        ...
        rew_reconstruction_loss_neighbor = []
        state_reconstruction_loss_neighbor = []
        ...
        for idx_traj in range(num_tasks):
            ...
            curr_actions_neighbor = vae_actions_neighbor[:, idx_traj, :]
            ...
            if len(np.unique(trajectory_lens)) == 1 and not self.args.decode_only_past:
                ...
                dec_actions_neighbor = curr_actions_neighbor.unsqueeze(0).expand((num_latents, *curr_actions_neighbor.shape))
            ...
            # (these are the different ELBO_t terms)
                ...
                for i, idx_timestep in enumerate(len_encoder[idx_traj]):
                    ...
                    if dec_until != 0:
                        ...
                        dec_actions_neighbor.append(curr_actions_neighbor[:dec_until])
                ...
                if self.args.decode_only_past:
                    ...
                    dec_actions_neighbor = torch.cat(dec_actions_neighbor)
                else:
                    ...
                    dec_actions_neighbor = torch.stack(dec_actions_neighbor)
            ...
            if self.args.decode_reward_neighbor:
                # compute reconstruction loss for this trajectory
                # (for each timestep that was encoded, decode everything and sum it up)
                rrc = self.compute_rew_reconstruction_loss_neighbor(dec_embedding,
                                                                    dec_prev_obs,
                                                                    dec_next_obs,
                                                                    dec_actions,
                                                                    dec_rewards,
                ...
                if self.args.decode_only_past:
                    ...
                    rrc = rrc.sum(dim=1)
                rew_reconstruction_loss_neighbor.append(rrc)
            if self.args.decode_state_neighbor:
                src = self.compute_state_reconstruction_loss_neighbor(dec_embedding,
                                                                      dec_prev_obs,
                                                                      dec_next_obs,
                                                                      dec_actions,
                                                                      dec_actions_neighbor
                ...
                src = src.sum(dim=1)
                state_reconstruction_loss_neighbor.append(src)
            if self.args.decode_task:
        ...
        if self.args.decode_reward_neighbor:
            rew_reconstruction_loss_neighbor = torch.stack(rew_reconstruction_loss_neighbor)
            rew_reconstruction_loss_neighbor = rew_reconstruction_loss_neighbor.sum(dim=1)
        else:
            rew_reconstruction_loss_neighbor = 0

        if self.args.decode_state_neighbor:
            state_reconstruction_loss_neighbor = torch.stack(state_reconstruction_loss_neighbor)
            state_reconstruction_loss_neighbor =  state_reconstruction_loss_neighbor.sum(dim=1)
        else:
            state_reconstruction_loss_neighbor = 0
        ...
        if self.args.use_neighbor:
            loss = (self.args.rew_loss_coeff * rew_reconstruction_loss +
                    self.args.state_loss_coeff * state_reconstruction_loss +
                    self.args.rew_loss_coeff_neighbor * rew_reconstruction_loss_neighbor +
                    self.args.state_loss_coeff_neighbor * state_reconstruction_loss_neighbor +
                    self.args.task_loss_coeff * task_reconstruction_loss +
                    self.args.kl_weight * kl_loss).mean()
        ...
        if self.args.decode_reward_neighbor:
            assert rew_reconstruction_loss_neighbor.requires_grad
        if self.args.decode_state_neighbor:
            assert state_reconstruction_loss_neighbor.requires_grad
        ...
        self.log(elbo_loss, rew_reconstruction_loss, state_reconstruction_loss, task_reconstruction_loss, kl_loss, rew_reconstruction_loss_neighbor, state_reconstruction_loss_neighbor)
    ...
    def log(self,
        ...
        if curr_iter_idx % self.args.log_interval == 0:
            ...
            if self.args.decode_reward_neighbor:
                self.logger.add('vae_losses/reward_reconstr_err_neighbor', rew_reconstruction_loss_neighbor.mean(), curr_iter_idx)
            if self.args.decode_state_neighbor:
                self.logger.add('vae_losses/state_reconstr_err_neighbor', state_reconstruction_loss_neighbor.mean(), curr_iter_idx)
