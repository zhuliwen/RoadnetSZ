...
# Arguments In Config File:
    - interval: time of each simulation step (in seconds). 
      An interval of 0.5 means for each simulation step, the system will move 0.5 seconds forward.
    ...
    - rlTrafficLight: whether to enable traffic light control through python API. 
      If set to false, default traffic light plan defined in roadnetFile will be used.
      
    - saveReplay: whether to save simulation for replay. 
      If set to true, roadnetLogFile and replayLogFile are required.
      
    - roadnetLogFile: path for roadnet replay file. 
      This is a special roadnet file for replay, not the same as roadnetFile
      
    - replayLogFile: path for replay. 
      This file contains vehicle positions and traffic light situation of each simulation step.
...
# Sample Config File:
    {
        ...
        "saveReplay": true,
        "roadnetLogFile": "log_roadnet.json",
        "replayLogFile": "log_replay.txt"
    }
"""
...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log):
        ...
        self.all_yellow_phase_index = 1 # -1
        self.all_red_phase_index = 1 # -2
        ...
        if self.all_yellow_flag:
            ...
            if action_pattern == "switch":  # switch by order
                if action == 0:  # keep the phase
                    ...
                    self.next_phase_to_set_index = mydict[self.current_phase_index]
                else:
            ...
            if self.current_phase_index == self.next_phase_to_set_index:  # the light phase keeps unchanged
                ...
                self.eng.set_tl_phase(self.inter_name, self.current_phase_index)  # !!! yellow, tmp
                path_to_log_file = os.path.join(self.path_to_log, "signal_inter_{0}.txt".format(self.inter_name))
                df = [self.get_current_time(), self.current_phase_index]
                df = pd.DataFrame(df)
                df = df.transpose()
                ...
                self.flicker = 1
        print("----- set_tl_phase ----- intersection ----- ", self.inter_name, " ----- phase ----- ",
    ...
    def _update_feature_map(self, simulator_state):
        ...
        dic_feature[
            "adjacency_matrix"] = self._get_adjacency_row()  # TODO this feature should be a dict? or list of lists

        dic_feature[
            "adjacency_matrix_lane"] = self._get_adjacency_row_lane()  # row: entering_lane # columns: [inputlanes, outputlanes]
...
class AnonEnv:
    ...
    def __init__(self, path_to_log, path_to_work_directory, dic_traffic_env_conf):
        ...
        self.global_dic_vehicles = {}
        self.all_vehicles_travel_time_dict = {}
        self._max_episode_steps = 900  # default 300 # 900
        self.round_index = 0
    ...
    def step(self, action):
        action = torch.Tensor.cpu(action).numpy().tolist()
        ...
        # print("get_average_travel_time: ", self.eng.get_average_travel_time())
        # print("get_vehicle_count", self.eng.get_vehicle_count())
        # print("list_action_in_sec_display", list_action_in_sec_display)
        return next_state, reward, done, {'task': [time.time() - step_start_time]}
    ...
    def _inner_step(self, action):
        ...
        print("get_current_time: ", self.eng.get_current_time())
        print("get_average_travel_time: ", self.eng.get_average_travel_time())
        print("get_vehicle_count: ", self.eng.get_vehicle_count())
        print()

        df = [self.round_index, self.eng.get_current_time(), self.eng.get_average_travel_time()]
        df = pd.DataFrame(df)
        df = df.transpose()
        df.columns = ["round", "get_current_time", "get_average_travel_time"]
        print(df)
        # df.to_csv("./zlwlog/Round_CurrentTime_TravelTime_{}.csv".format(time.strftime('%m_%d', time.localtime(time.time()))), mode='a', header=False)
        args = args_cityflow_vae.get_args_3_3_raw()
        df.to_csv(
            "./zlwlog/TravelTime_{0}_{1}.csv".format(args.log_name, (time.strftime('%m_%d', time.localtime(time.time())))),
            mode='a', header=False)

        if self.eng.get_current_time() == 3600:
            self.reset()
            self.round_index += 1
            print("-------------------------- round ", self.round_index, " --------------------------")
        ...
        if self.dic_traffic_env_conf["FAST_COMPUTE"]:
            ...
            '''
            by_lane = self.eng.get_lane_vehicles()
            vehicle_id_by_lane = []
            [vehicle_id_by_lane.extend(vehicle_id) for vehicle_id in by_lane.values()]
            vehicle_id_network = self.eng.get_vehicles(include_waiting=False)
            if len(vehicle_id_by_lane) != len(vehicle_id_network):
                print('!!!different!!!', 'eng.get_lane_vehicles(): ', len(vehicle_id_by_lane), '————', 'eng.get_vehicles(): ', len(vehicle_id_network))
            else:
                print('eng.get_lane_vehicles(): ', len(vehicle_id_by_lane), '————', 'eng.get_vehicles(): ', len(vehicle_id_network))
            print('``````````````````')
            '''
        # print("Get system state time: ", time.time()-system_state_start_time)
        ...
        global_dic_vehicle_set = set(self.global_dic_vehicles.keys())  # old list
        list_new_vehicles = self.eng.get_vehicles(include_waiting=True)
        ...
        for vehicle_id in global_dic_vehicle_set - global_vehicle_id_set:  # it has left the whole network
            self.all_vehicles_travel_time_dict[vehicle_id] = self.eng.get_current_time() - self.global_dic_vehicles[
                vehicle_id]
            del (self.global_dic_vehicles[vehicle_id])  # old -= (old-new): old = old intersect new

        for vehicle_id in global_vehicle_id_set - global_dic_vehicle_set:
            self.global_dic_vehicles[vehicle_id] = self.eng.get_current_time()
    ...
    def batch_log(self, start, stop):
        for inter_ind in range(start, stop):
            ...
            f.close()
        print('average travel time from cityflow API: ', self.eng.get_average_travel_time())
        if self.all_vehicles_travel_time_dict:
            average_travel_time_global = np.mean(list(self.all_vehicles_travel_time_dict.values()))
            print('Their average travel time from global: %f' % np.mean(
                list(self.all_vehicles_travel_time_dict.values())))
        else:
            average_travel_time_global = '999999999999999999999'
            print('Their average travel time: ', '999999999999999999999')
        memory_str_global = '%d vehicles left\taverage_travel_time = %f' % (
        len(self.all_vehicles_travel_time_dict), average_travel_time_global)
        memory_str_cityflow = 'average_travel_time_cityflow = %f' % self.eng.get_average_travel_time()
        f_average_travel_time = open(
            os.path.join(self.path_to_log, "travel_time_{}.txt".format(self.dic_traffic_env_conf['TRAFFIC_FILE'])), "a")
        f_average_travel_time.write(memory_str_global + "\n")
...
def convert_to_input(state, dic_traffic_env_conf):
    ...
    if state["cur_phase"][0] == 0:
        state["cur_phase"][0] = np.random.randint(1, 8)
    start_lane = dic_traffic_env_conf["LANE_PHASE_INFO"]["phase_startLane_mapping"][state["cur_phase"][0]]
    print("-------------- state-cur_phase-0 ------------", state["cur_phase"][0])
...
if __name__ == '__main__':
    ...
    dic_exp_conf = {
        ...
        "ROADNET_FILE": "roadnet_{0}.json".format("4_4"),
    ...
    dic_path = {
        "PATH_TO_MODEL": "/home/zlw/PycharmProjects/meta_vim/model/test/hangzhou_test",
        "PATH_TO_WORK_DIRECTORY": "/home/zlw/PycharmProjects/meta_vim/records/test/hangzhou_test",
