...
class RolloutStorageVAE(object):
    def __init__(self, num_processes, max_trajectory_len, zero_pad, max_num_rollouts,
        ...
        self.actions_neighbor = torch.zeros((self.max_traj_len, self.max_buffer_size, action_dim))
        ...
        self.running_actions_neighbor = [[] for _ in range(num_processes)]
        if self.tasks is not None:
            self.running_tasks = [None for _ in range(num_processes)]

        # print("-------------------- init running_prev_obs ------------------------\n", self.running_prev_obs)
        # print("-------------------- init running_next_obs ------------------------\n", self.running_next_obs)
        # print("-------------------- init running_rewards ------------------------\n", self.running_rewards)
    ...
    def get_running_batch(self):
        ...
        lengths = []
        actions_neighbor = []
        # print("-------------------- running_prev_obs ------------------------\n", len(self.running_prev_obs[0]))
        # print("-------------------- running_next_obs ------------------------\n", len(self.running_next_obs[0]))
        # print("-------------------- running_rewards ------------------------\n", len(self.running_rewards[0]))
        # print("-------------------- running_actions ------------------------\n", len(self.running_actions[0]))
        for i in range(self.num_processes):
            pad_len = self.max_traj_len - len(self.running_prev_obs[i])
            pad_len = 180
            # print("-------------- pad_len ------------", pad_len, "-------------- max_traj_len ------------", self.max_traj_len)
            ...
            if pad_len > 0 and len(self.running_prev_obs[i]) > 0:
                ...
                actions_neighbor.append(torch.cat((torch.cat(self.running_actions_neighbor[i]),
                                                   torch.zeros(pad_len, self.running_actions_neighbor[i][0].shape[1]).to(device))))
            else:
                # print("---------------- enter 2 ---------------")
                ...
                actions_neighbor.append(torch.zeros(pad_len, self.action_dim).to(device))
        ...
        return torch.stack(prev_obs, dim=1), \
               ...
               torch.stack(actions_neighbor, dim=1), \
               lengths

    def insert(self, prev_obs, actions, next_obs, rewards, reset_task, task, actions_neighbor):

        actions = actions
        actions_neighbor = actions_neighbor
        rewards = rewards.reshape(list(rewards.shape)[0], 1)
        for i in range(self.num_processes):
            actions[i] = actions[i]
            ...
            for j in range(1, 5):
                self.running_actions_neighbor[i].append(actions_neighbor[j][i].unsqueeze(0))
    ...
    def get_batch(self, num_enc_len=1, num_rollouts=5, include_final=True, replace=False):
        ...
        actions_neighbor = self.actions_neighbor[:, rollout_indices, :]
        if self.tasks is not None:
        ...
        return prev_obs.to(device), next_obs.to(device), actions.to(device), \
               rewards.to(device), tasks, len_encoder, trajectory_lens, actions_neighbor.to(device)
