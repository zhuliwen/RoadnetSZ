...
def env_step(envs, action, dic_traffic_env_conf):
    next_obs, reward, done, infos = envs.step(action.detach())

    next_obs = [convert_to_input(i, dic_traffic_env_conf) for i in next_obs]
...
def get_augmented_obs(args, obs,
    ...
    if sample_embeddings and (latent_sample is not None):
        ...
        latent_mean = latent_mean.reshape((-1, latent_mean.shape[-1])) # -1
...
def update_encoding(encoder, next_obs, action, reward, done, hidden_state):
    reward = reward.reshape(list(reward.shape)[0], 1)
