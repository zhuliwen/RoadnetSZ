...
class CoLightAgent(Agent): 
    def __init__(self, 
        ...
        if cnt_round == 0: 
            ...
            if self.dic_path["PATH_TO_RELOAD_MODEL"] is not None and os.listdir(self.dic_path["PATH_TO_RELOAD_MODEL"]):
                ...
                file_name = "inter_{0}".format(self.intersection_id)
                ...
                file_name_bar = "inter_{0}_bar".format(self.intersection_id)
                self.q_network_bar = load_model(
                    os.path.join(file_path, "%s.h5" % file_name_bar),
                    custom_objects={'RepeatVector3D':RepeatVector3D, 'L2ReLURegularizer':L2ReLURegularizer})
                print("succeed in reloading model for initialization from %s"%self.dic_path["PATH_TO_RELOAD_MODEL"])
            else:
                self.q_network = self.build_network()
                self.q_network_bar = self.build_network_from_copy(self.q_network)
        else:
    ...
    def adjacency_index2matrix(self,adjacency_index):
        ...
        adjacency_index[adjacency_index==-1] = self.num_agents
        adjacency_index_new=np.sort(adjacency_index,axis=-1)
        adjacency_index[adjacency_index==self.num_agents] = -1
        l = to_categorical(adjacency_index_new,num_classes=self.num_agents+1)
        l = l[:,:,:,:-1]
        return l
    ...
    def build_network_from_copy(self, network_copy):
        ...
        network = model_from_json(network_structure, custom_objects={"RepeatVector3D": RepeatVector3D, 'L2ReLURegularizer':L2ReLURegularizer})
    ...
    def save_network_bar(self, file_name):
        # --- my modification ---
        critic_weights = self.q_network.get_weights()
        critic_target_weights = self.q_network_bar.get_weights()
        for i in range(len(critic_weights)):
            critic_target_weights[i] = self.dic_agent_conf["TAU"] * critic_weights[i] + (1-self.dic_agent_conf["TAU"])* critic_target_weights[i]
        self.q_network_bar.set_weights(critic_target_weights)
