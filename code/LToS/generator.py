        ...
        while not done and step_num < int(self.dic_exp_conf["RUN_COUNTS"]/self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            for i in range(self.dic_traffic_env_conf["NUM_AGENTS"]):
                ...
                if self.dic_exp_conf["MODEL_NAME"] in ["SwarmCoLight"]:
                    one_state = state
                    if step_num % self.dic_traffic_env_conf["MIN_W_STEP"] == 0:
                        w_in, _ = self.agents[i].choose_w_in(step_num, one_state)
                        w_in_list = w_in
                    # else: same w_in
                    action, _ = self.agents[i].choose_action(step_num, one_state, w_in)
                    action_list = action
                elif self.dic_exp_conf["MODEL_NAME"] in ["CoLight","GCN", "SimpleDQNOne"]:
                    one_state = state
            ...
            if self.dic_exp_conf["MODEL_NAME"] in ["SwarmCoLight"]:
                next_state, reward, done, _ = self.env.step_with_w_in(action_list, w_in_list)
            else:
