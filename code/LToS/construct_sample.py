...
class ConstructSample:
    ...
    def __init__(self, path_to_samples, cnt_round, dic_traffic_env_conf, model_name):
    ...
    # w_in
    def judge_w_in(self,time,i):
        if self.logging_data_list_per_gen[i][time]['w_in'][0] == -1:
            raise ValueError
        else:
            return self.logging_data_list_per_gen[i][time]['w_in']
    ...
    # w_in
    def make_reward_with_w_in(self, folder):
        '''
        make reward for one folder and all intersections,
        add the samples of one intersection into the list.samples_all_intersection
        :return:
        '''
        for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
            if self.samples_all_intersection[i] is None:
        ...
        try:
            total_time = np.min([int(self.logging_data_list_per_gen[i][-1]['time'] + 1)
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS'])])
            # construct samples
            time_count = 0
            ...
            for time in range(0, total_time - self.measure_time + 1, self.interval):
                ...
                # exchange reward
                full_adj_matrix = np.zeros((num_agents, num_agents), dtype=np.int)
                list_samples = []
                for i in range(num_agents):
                    ...
                    if time + self.interval == total_time:
                        ...
                        next_state = self.construct_state(self.dic_traffic_env_conf["LIST_STATE_FEATURE"],
                                                          time + self.interval, i)
                    sample = [state, w_in, action, next_state, reward_average, reward_instant, time,
                              folder+"-"+"round_{0}".format(self.cnt_round)]
                    list_samples.append(sample)
                    # build full_adj_matrix to transfer w_out to w_in
                    raw_adj = state["adjacency_matrix"]
                    raw_adj = np.array(raw_adj, dtype=np.int)
                    out_neighbors = raw_adj[raw_adj != -1]
                    full_adj_matrix[i, out_neighbors] = 1

                full_adj_matrix = np.transpose(full_adj_matrix, (1,0))
                for i in range(num_agents):
                    state, w_in, action, next_state, _, _, _, path = list_samples[i]
                    exchange_reward_average, exchange_reward_instant = 0, 0
                    in_neighbors = np.where(full_adj_matrix[i,:] == 1)[0]
                    for j, neighbor_ind in enumerate(in_neighbors):
                        neighbor_reward_average, neighbor_reward_instant = list_samples[neighbor_ind][4], list_samples[neighbor_ind][5]
                        exchange_reward_average += w_in[j] * neighbor_reward_average
                        exchange_reward_instant += w_in[j] * neighbor_reward_instant
                    sample = [state, w_in, action, next_state, exchange_reward_average, exchange_reward_instant, time, path]
                    # sample = self.evaluate_sample(sample)
                    self.samples_all_intersection[i].append(sample)
            return 1
        except Exception as e:
            print("Error occurs when making rewards in generator {0} for intersection {1}".format(folder, i))
            print('traceback.format_exc():\n%s' % traceback.format_exc())
    ...
    def make_reward_for_system(self):
        ...
        for folder in os.listdir(self.path_to_samples):
            ...
            # w_in
            if self.model_name in ["SwarmCoLight"]:
                pass_code = self.make_reward_with_w_in(folder)
                if pass_code == 0:
                    pass
            else:
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
...
if __name__=="__main__":
    ...
    cs = ConstructSample(path_to_samples, 0, dic_traffic_env_conf, "SwarmCoLight")
