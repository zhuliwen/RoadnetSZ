...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict,path_to_log):
        ...
        # w_in
        self.in_adjacency_row = light_id_dict['in_adjacency_row']
    ...
    def _update_feature_map(self, simulator_state):
        ...
        # w_in
        dic_feature["in_adjacency_matrix"] = self._get_in_adjacency_row()
    ...
    # w_in
    def _get_in_adjacency_row(self):
        return self.in_adjacency_row
...
class AnonEnv:
    ...
    def reset(self):
        ...
        if not self.dic_traffic_env_conf['ADJACENCY_BY_CONNECTION_OR_GEO']:
            self._get_in_adjacency_extraction()
        else:
            for i in self.traffic_light_node_dict.keys():
                raw_total_adjs = np.copy(self.traffic_light_node_dict[i]['adjacency_row'])
                ...
                self.traffic_light_node_dict[i]['in_adjacency_row'] = raw_total_adjs
    ...
    # w_in
    def step_with_w_in(self, action, w_in):
        step_start_time = time.time()
        list_action_in_sec = [action]
        list_action_in_sec_display = [action]
        # w_in
        list_w_in_in_sec_display = [w_in.tolist()]
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]-1):
            if self.dic_traffic_env_conf["ACTION_PATTERN"] == "switch":
                list_action_in_sec.append(np.zeros_like(action).tolist())
            elif self.dic_traffic_env_conf["ACTION_PATTERN"] == "set":
            ...
            list_action_in_sec_display.append(np.full_like(action, fill_value=-1).tolist())
            # w_in
            list_w_in_in_sec_display.append(np.full_like(w_in, fill_value=-1).tolist())
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            action_in_sec_display = list_action_in_sec_display[i]
            # w_in
            w_in_in_sec_display = list_w_in_in_sec_display[i]
            ...
            # log
            self.log_with_w_in(cur_time=instant_time, before_action_feature=before_action_feature, action=action_in_sec_display,
                w_in=w_in_in_sec_display)
    ...
    # w_in
    def log_with_w_in(self, cur_time, before_action_feature, action, w_in):
    ...
    # w_in
    def _get_in_adjacency_extraction(self):
        """
        out-neighbors to in-neighbors
        """

        total_in_neighbors = []
        num_agents = len(self.traffic_light_node_dict)
        full_adj_matrix = -np.ones((num_agents,num_agents))
        for inter_ind, i in enumerate(self.traffic_light_node_dict.keys()):
            raw_total_adjs=np.array(self.traffic_light_node_dict[i]['adjacency_row'], dtype=np.int)
            raw_total_adjs[raw_total_adjs==-1] = 1e8
            raw_total_adjs=np.sort(raw_total_adjs,axis=-1)
            raw_total_adjs[raw_total_adjs==1e8] = -1
            # still put all -1 at the tail
            out_neighbors = raw_total_adjs[raw_total_adjs > 0]
            full_adj_matrix[inter_ind,out_neighbors] = 1
            # illegal places remain -1

        total_in_neighbors = []
        full_adj_matrix = np.transpose(full_adj_matrix, (1,0))
        for inter_ind in range(num_agents):
            # in-neighbors might be more than limited
            in_neighbors = np.where(full_adj_matrix[inter_ind,:] > 0)[0]
            total_in_neighbors.append(in_neighbors.tolist())
        # padding -1
        max_len = np.max([len(l) for l in total_in_neighbors])
        padding_in_neighbors = list(map(lambda l:l + 
            [-1 for _ in range(max_len-len(l))], total_in_neighbors))
        padding_in_neighbors = np.array(padding_in_neighbors, dtype=np.int)
        
        for inter_ind, i in enumerate(self.traffic_light_node_dict.keys()):
            self.traffic_light_node_dict[i]['in_adjacency_row'] = padding_in_neighbors[inter_ind]
