...
class Updater:
    ...
    def __init__(self, cnt_round, dic_agent_conf, dic_exp_conf, dic_traffic_env_conf, dic_path, best_round=None, bar_round=None):
        ...
        # w_in
        if self.dic_exp_conf['MODEL_NAME'] in ["SwarmCoLight"]:
            self.full_columns = ['state','w_in','action','next_state','inst_reward','reward','time','generator']
            self.brief_columns = ['state','w_in','action','next_state','inst_reward','reward']
        else:
            self.full_columns = ['state','action','next_state','inst_reward','reward','time','generator']
            self.brief_columns = ['state','action','next_state','inst_reward','reward']
    ...
    def load_sample_for_agents(self):
        ...
        if self.dic_exp_conf['MODEL_NAME'] not in ["GCN","CoLight","SwarmCoLight"]:
            ...
            if False: # Todo decide multi-process
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    ...
                    if len(sample_set) == 0:
                        continue
                    samples_set_df = pd.DataFrame.from_records(sample_set,columns= self.full_columns)
                    samples_set_df['input'] = samples_set_df[self.brief_columns].values.tolist()
                    samples_set_df.drop(self.brief_columns, axis=1, inplace=True)
                ...
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    ...
                    samples_set_df = pd.DataFrame.from_records(sample_set,columns= self.full_columns)
                    samples_set_df['input'] = samples_set_df[self.brief_columns].values.tolist()
                    samples_set_df.drop(self.full_columns, axis=1, inplace=True)
                    # samples_set_df['inter_id'] = i
    ...
    def update_network(self,i):
        ...
        # w_in
        if self.dic_exp_conf['MODEL_NAME'] in ["SwarmCoLight"]:
            if (self.cnt_round+1) % self.dic_agent_conf['UPDATE_W_FREQ'] == 0:
                self.agents[i].train_w_network(self.dic_exp_conf)
        if self.dic_traffic_env_conf["ONE_MODEL"]:
            ...
            if self.dic_exp_conf["PRETRAIN"]:
                ...
                self.agents[i].save_network_bar("round_{0}_inter_{1}_bar".format(self.cnt_round,self.agents[i].intersection_id))
                # w_in
                if self.dic_exp_conf['MODEL_NAME'] in ["SwarmCoLight"]:
                    # still have to save w network in every round
                    self.agents[i].save_w_network("round_{0}_inter_{1}_w".format(self.cnt_round,self.agents[i].intersection_id))
                    self.agents[i].save_w_network_bar("round_{0}_inter_{1}_w_bar".format(self.cnt_round,self.agents[i].intersection_id))
